...
<?php $this->load->view('groupuser/leftsidebar.php');?>
  ...
  <!-- ========= Main Content Start ========= -->
	...
	?>---->
		...
		<form id="wowza-form" class="form-only form-one" method="post" action="<?php echo site_url();?>groupuser/updateUserdetails" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<select class="form-control select" name="role_id" id="role_id" disabled>
				...
				<option value="3" selected>Group User</option>
			</select>
		</div>
		<!--<div class="form-group">
			<label>Role</label>
			<select class="form-control select" name="role_id" id="role_id">
				<?php if(!empty($groupuser)){?>
					...
					<option value="3" selected="selected"><?php if($groupuser[0]['role_id'] == '1'){echo "Admin";}?></option>
