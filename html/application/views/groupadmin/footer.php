   <footer class="main-footer">
	...
	$(function() {
	  /* Upload Profile Pic */
		$('#profilePic').dropzone({ url: baseURL + "groupadmin/uploadGroupPic",uploadMultiple:false,dictDefaultMessage:"Click / Drop here to upload files"});	
	});
	$('#uploadprofilepic').click(function(){ $('#groupfile').trigger('click'); });
	...
	{
		$.ajax({
			url:baseURL + "groupadmin/getApplicationStreams",
			type:"post",
			data:{'id':id},
			dataType:"json",
	...
	function showAddress(ip)
	{
		...
		else
		{
			...
			{
				$.ajax({
				url:baseURL + "groupadmin/getIPAdressByWowaz",
				type:"post",
				data:{'id':ip},
				dataType:"json",
				success:function(jsonResponse){
		...
		if($isLock)
		{
			?>
				<div id="divlock" class="screenunlock">
					<?php
					$this->load->view('groupadmin/login');
					?>
				</div>
			<?php
		}
