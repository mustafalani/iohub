function initShareButtons() {
    ...
    init('.hp-social-gp', 550, 500, 'https://plus.google.com/share?url=' + url);
}
...
function playM3U8(src) {
    ...
    if ($('#player-proxy').prop('checked')) {
        playM3U8byGrindPlayer(src.replace(/https?:\/\//i, 'https://localhost/fetch.php/'));
    } else {
...
function playTargetRTMP(src,pid) {
    $('#player-tip').hide();
    if (isMobile()) {
    ...
    {
		swfobject.embedSWF(baseURL + 'public/site/main/js/GrindPlayer.swf', pid, '100%', '386', '10.2', null, flashvars, params, attributes);
	}
	else
	{
		swfobject.embedSWF(baseURL + 'public/site/main/js/GrindPlayer.swf', pid, '100%', '300', '10.2', null, flashvars, params, attributes);	
	}
    
}
function playRTMP(src,pid) {
    $('#player-tip').hide();
    if (isMobile()) {
        $('#player-tip').html("RTMP protocol is not supported on your device.");
        return;
    ...
    if(Action == "editTarget")
    {
		swfobject.embedSWF(baseURL + 'public/site/main/js/GrindPlayer.swf', pid, '100%', '405', '10.2', null, flashvars, params, attributes);
	}
	else
	{
		swfobject.embedSWF(baseURL + 'public/site/main/js/GrindPlayer.swf', pid, '100%', '300', '10.2', null, flashvars, params, attributes);	
