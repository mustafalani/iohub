    ...
    $.widget("flowchart.flowchart", {
        ...
        createLink: function (linkId, linkDataOriginal) {    
            ...
            var toSubConnector = subConnectors[1];	
             var linksFromArray = linkDataOriginal.fromOperator.split('_');
      		var linksToArray = linkDataOriginal.toOperator.split('_');
            if(linksFromArray[1] == 'Output')
            {
            	if(linksFromArray[2] == "srt")
            	{
					var multipleLinksOnOutput = false;
				}
				else
				{
					var multipleLinksOnOutput = true;	
				}
				
			}
			else if(linksFromArray[1] == 'Gateway' || linksFromArray[1] == 'Publisher')
			{
				var multipleLinksOnOutput = true;	
			}
			else
			...
			if(linksFromArray[2] == "sdi" && linksToArray[2] == "sdi")
			{
				toastr['error']('You cant add link between SDI input to SDI Output');
				return;
			}
			if(linksFromArray[1] != "Publisher" && linksToArray[0] == "Target")
			{
				toastr['error']('You cant add link this to Target');
				return;
			}
			if(linksFromArray[2] != "ndi" && linksToArray[1] == "Gateway")
			{
				toastr['error']('You cant add link between ' + linksFromArray[2] + ' input to Gateway');
				return;
			}
	  		if(_settings[linkDataOriginal.fromOperator].settings == false || _settings[linkDataOriginal.toOperator].settings == false)
	  		{
			 	toastr['error']('You cant create link between INPUT/OUTPUT, until you set input/ouptup properties');
				return;
			}
            ...
            $('.table_priority tbody').html("");
            var linkcounter = 0;
            for(var k in this.data.links)
            {
            	var objlink = {};
            	var position = 0;
            	var positin_priority =0;
            	objlink.from = this.data.links[k].fromOperator;
            	objlink.to = this.data.links[k].toOperator;
				_settings['links'].push(objlink);
				
				for(var b in this.data.links)
            	{
					if(this.data.links[b].fromOperator == this.data.links[k].fromOperator)
					{
						positin_priority = position;
						break;
					}
					position++;					
				}			
				pri.push(positin_priority);	
				var frmar = this.data.links[k].fromOperator.split('_');
				var toar = this.data.links[k].toOperator.split('_');
				var r = "<tr>";
				r += "<td>CH" + (cntr + 1) + "</td>";
				r += "<td>" + frmar[2].toUpperCase() + "/" + positin_priority +  "</td>";
				r += "<td>" + toar[2].toUpperCase() + "/" + positin_priority + "/" + (cntr + 1) + "</td>";
				var jsonEncoders = allEncoders;
				r += "<td>";
				if(cntr == 0)
				{
					var obj = _settings[this.data.links[k].fromOperator];
					var encids = obj.channel_encoder;
					encdrids = encids.split('_');
					encid = encdrids[1];
					for(var key in jsonEncoders)
					{
						var e = JSON.parse(jsonEncoders[key]);
						console.log(e);
						if(e.id == encid)
						{
							r += e.encoder_name;
				...
				else
				{
					...
					else if(frmar[1] == "Output")
					{
						var obj = _settings[this.data.links[k].toOperator];						
						var encids = obj.channel_out_encoder;
						encdrids = encids.split('_');
						encid = encdrids[1];
						if(encid > 0)
						{
					...
					//var obj1 = _settings[this.data.links[k].toOperator];
					*/
				}
				r += "</td>";
				if(toar[2].toUpperCase() == "GATEWAY")
				{
					r += "<td>" + frmar[2].toUpperCase() + "TONDI</td>";
				}
				else if(toar[2].toUpperCase() == "PUBLISHER")
				{
					r += "<td>" + frmar[2].toUpperCase() + "TORTMP</td>";
				}
				else if(frmar[2].toUpperCase() == "GATEWAY")
				{
					r += "<td>NDITO" + toar[2].toUpperCase() + "</td>";
				}
				else if(frmar[2].toUpperCase() == "PUBLISHER")
				{
					r += "<td>RTMPTO" + toar[2].toUpperCase() + "</td>";
				}
				else
				{
					r += "<td>" + frmar[2].toUpperCase() + "TO" + toar[2].toUpperCase() + "</td>";
				}				
				if(cntr == 0)
				{
					r += "<td>" + positin_priority + "</td>";	
				}
				else
				{		
					var counts ={};			
					...
					if(counts[positin_priority] == 1)
					{
						r += "<td>" + (positin_priority * 10) + "</td>";	
					}
					else if(counts[positin_priority] > 1)
					{
						var pp = parseInt(positin_priority * 10) + parseInt(counts[positin_priority]-1);
						r += "<td>" + pp + "</td>";
					}					
				}
				...
				r += "</tr>";
				cntr++;
				$('.table_priority tbody').append(r);
				if(linkcounter > 0)
				{
					if(toar[1].toUpperCase() == "OUTPUT")
					{
						if(this.data.links[k].toOperator == linkDataOriginal.toOperator)
						{
							switch(toar[2].toUpperCase())
							{
								case "NDI":							
								...
								_settings[this.data.links[k].toOperator].settings = false;
        ...
        _getOperatorFullElement: function (operatorData) {
			...
			var ids = infos.id.split('_');
			
			 var $operator_title = $('<div class="flowchart-operator-title"></div>');
			if(ids[1] == "input")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator inputbox"></div>');	
				$operator_title.html(infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("input_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "output")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator outputbox"></div>');
				$operator_title.html(infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("output_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "gateway")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator gatewaybox"></div>');
				$operator_title.html(infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("gateway_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "publisher")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator publisherbox"></div>');
				$operator_title.html(infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("publishers_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "facebook")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator facebookbox"></div>');
				$operator_title.html('<i class="fa fa-facebook"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "youtube")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator youtubebox"></div>');
				$operator_title.html('<i class="fa fa-youtube"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "twitch")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator twitchbox"></div>');
				$operator_title.html('<i class="fa fa-twitch"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "twitter")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator twitterbox"></div>');
				$operator_title.html('<i class="fa fa-twitter"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "wowzacdn")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator wowzacdnbox"></div>');
				$operator_title.html('<i class="fa fa-cloud"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "TRGRTMP")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator TRG_RTMPbox"></div>');
				$operator_title.html('<i class="fa fa-gear"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "TRGMPEGTS")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator TRG_MPEG_TSbox"></div>');
				$operator_title.html('<i class="fa fa-gear"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "TRGRTP")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator TRG_RTPbox"></div>');
				$operator_title.html('<i class="fa fa-gear"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else if(ids[1] == "TRGSRT")
			{
				var $operator = $('<div id="'+infos.name+'" class="flowchart-operator TRG_SRTbox"></div>');
				$operator_title.html('<i class="fa fa-gear"></i> ' + infos.title + ' <a href="javascript:void(0);" onclick=workflow_settings("TRG_'+ids[2].trim()+'","'+infos.name+'");><i class="fa fa-cogs" aria-hidden="true"></i></a>');
			}
			else
			{
           ...
           $closeLink = $('<a class="cls_close_wf" href="javascript:void(0);"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a>');
            
            $closeLink.appendTo($operator);
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            this.data.operators[operatorId].internal.els = fullElement;
			 ...
			 _settings[operatorId].x = operatorData.left;
             _settings[operatorId].y = operatorData.top;
            ...
            function operatorChangedPosition(operator_id, pos) {
                ...
                _settings[operator_id].x = pos.left;
                _settings[operator_id].y = pos.top;
        ...
        _deleteLink: function (linkId, forced) {
            ...
            for(var k in this.data.links)
            {
				...
				var r = "<tr>";
				r += "<td>CH" + (cntr + 1) + "</td>";
				r += "<td>" + frmar[2].toUpperCase() + "/" + cntr +  "</td>";
				r += "<td>" + toar[2].toUpperCase() + "/" + cntr + "/" + (cntr + 1) + "</td>";
				r += "<td>" + frmar[2].toUpperCase() + "TO" + toar[2].toUpperCase() + "</td>";
				r += "<td>" + cntr + "</td>";
				r += "</tr>";
				cntr++;
