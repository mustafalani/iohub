...
var KURRENTTV_BASE_URL = 'https://kurrenttv.nbla.cloud';
...
$(document).on('click','.grid',function(){
	...
	$(tabhref + '_list').removeAttr('style');
	$(tabhref + '_grid').attr('style','display:flex');
	$(tabhref + '_grid').removeClass('dnone');
	...
	$.ajax({
        ...
        success:function(jsonResponse){
	        ...
	        if(OBJ.count >0)
	        {
	        	...
	        	{
					HTMLL += "<div class='mb-4 col-12 col-sm-6 col-md-4 col-xl-2'>";
					HTMLL += "<table class='w-100' style='table-layout: fixed;'>";
					HTMLL += "<tbody>";
					HTMLL += "<tr>";
					HTMLL += "<td>";
					HTMLL += "<div style='position: relative;'>";
					HTMLL += "<div>";
					HTMLL += "<div class='rh5v-DefaultPlayer_component ' title='"+ OBJ.data[i]['id'] +"'>";
					...
					HTMLL += "</div>";
					HTMLL += "</div>";
					HTMLL += "<a href='" + baseURL +"editasset/"+ OBJ.data[i]['id'] +"/" + nid + "'><h5 class='vidtitle'>"+ OBJ.data[i]['title'] +"</h5></a>";
					if (OBJ.data[i].hasOwnProperty("qc/state"))
					{
						switch(OBJ.data[i]['qc/state']){
							case 3:
							HTMLL += "<i class='icon-close icons text-danger'></i>";
								break;
							case 4:
							HTMLL += "<i class='icon-check icons text-success'></i>";
								break;
							case 0:
					...
					HTMLL += "<button class='badge badge-block btn-outline-secondary' disabled=''>" + fpsFormat(OBJ.data[i]['video/fps'])+ "FPS</button><span>  </span>";
					HTMLL += "<button class='badge badge-block btn-outline-secondary' disabled=''>" + videoCodec(OBJ.data[i]['video/codec'])+ "</button><span>  </span>";
					HTMLL += "<button class='badge badge-block btn-outline-secondary' disabled=''>" + OBJ.data[i]['video/width'] + "x" + OBJ.data[0]['video/height'] + "</button>";
					HTMLL += "</td>";
					HTMLL += "</tr>";
					HTMLL += "</tbody>";
					HTMLL += "</table>";
...
$(document).ready(function(){
	...
	/*$('.grid').each(function(){
		...
		//$('#' + idd + "_grid").hide();
		$('#' + idd + "_grid").parent('.card-body').attr('style','padding:0 !important');
	});
*/
...
$(document).on('click','.appstatrefresh',function(){
	$(this).find('i').remove();
	$(this).html("<img style='float:right;' src='"+baseURL +"public/site/main/images/loadSource.gif'/>");
	var obj = $(this);
	var ID = $(this).attr('id');
	var ids = ID.split('_');
	$.ajax({
...
$(document).on('click','.pairing_gateway_encoder',function(){
	...
	if(id != "")
	{
		...
		function(){
				$(this).find('span').html("<img src='"+baseURL +"public/site/main/images/loadgateway.gif'/>");
				$.ajax({
				...
				$.ajax({
					...
					success:function(jsonResponseExtreact){
						...
						if(jsonResponseExtreact.data.isExist == true)
						{
							...
							$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').find('img').attr('src',baseURL + 'public/site/main/tmp/images/' + jsonResponseExtreact.data.name);
						}
						else if(jsonResponseExtreact.data.isExist == false)
						{
							$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').find('img').attr('src',"");
							$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').find('img').attr('src',baseURL + 'public/site/main/images/default-gateway.png');
						}
					}
				});
			}
...
$(document).ready(function(){
	...
	/*archiveTargetTable = $('.archiveTargetTable').DataTable( {
	        ...
	        "ajax": baseURL + "admin/getarchiveTargets",
	         oLanguage: {sProcessing: "<div id='loaderlogs'><img src='"+baseURL+"/public/site/main/img/loading.gif'></div>"}
	    } );
...
$(document).on('click','.addBank',function(){
	...
	$(this).find('span').html("<img src='"+baseURL +"public/site/main/images/loadgateway.gif'/> Bank");
	$.ajax({
...
$(document).on('click','.resourceInfo',function(){
	...
	$.ajax({
		...
		success:function(jsonResponseExtreact){
			...
			if(jsonResponseExtreact.data.isExist == true)
			{
				$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').css('min-height','105px').find('img').attr('src',baseURL + 'public/site/main/tmp/images/' + jsonResponseExtreact.data.name);
				var info = "<ul>";
			...
			{
				$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').css('min-height','105px').find('img').attr('src',baseURL + 'public/site/main/images/default-gateway.png');
				var info = "<ul>";
				info += "<li>" + jsonResponseExtreact.data.Audio + "</li>";
				info += "<li>" + jsonResponseExtreact.data.Video + "</li>";
				$('#external-events div[name="'+jsonResponseExtreact.data.src+'"]').find('.infoSources').html(info);
...
$(document).on('click','.refreshSource',function(){
	...
	var anch = $(this);
	$(this).find('span').html("<img src='"+baseURL +"public/site/main/images/loadgateway.gif'/> Refresh");
	$.ajax({
			...
			switch(typpe)
			{
				...
				case "virinput":
					...
					switch(inpval)
					{
						...
						var Ecid = $("#channelEncoders").val();
						$('#find_NDISources').html("<img style='float:right;' src='"+baseURL +"public/site/main/images/loadSource.gif'/>");
						$.ajax({
							url: baseURL + "admin/getNDISource",
							data:{'id':Ecid,'txt':txt},
							type:'post',
...
$(document).on('click','.playbtn',function(){
	...
	var tit = $(this).parent().parent().parent().parent('td').find('.vidtitle').html();
	var srcc = $(this).attr('accesskey') + '/0000/'+ id + '.mp4';
	$('#myModal').find('.modal-title').html(tit);
	$('#myModal').find('source').attr('src',srcc);
	var vid = document.getElementById("videoTAG");
	vid.autoplay = true;
	vid.load();
});
$("#myModal button.close").on("click", function () {
    $('#myModal').find('source').attr('src',"");
    var vid = document.getElementById("videoTAG");
	vid.autoplay = false;
	vid.load(false);
});
...
$(document).ready(function(){
	var r = new Resumable({
	  target : 'https://kurrenttv.nbla.cloud/upload',
      query : {id_asset : 43,'session_id':'2b74c629c4044b4fe5680b2e8200a18e6e83cc6d'}
	});
